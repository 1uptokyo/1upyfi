...
@external
def setRewardPool(rewardPool: RewardPool):
...
def modify_lock(amount: uint256, unlock_time: uint256, user: address = msg.sender) -> LockedBalance:
    ...
    if msg.sender == user:
        if unlock_time != 0:
            ...
            assert ((unlock_week - self.round_to_week(block.timestamp)) / WEEK) < MAX_N_WEEKS, "Max week" # lock can't exceed 10 years
            assert unlock_week > block.timestamp, "Future"  #  dev: unlock time must be in the future
            if unlock_week - block.timestamp < MAX_LOCK_DURATION:
                assert unlock_week > old_lock.end, "only increase duration"  # dev: can only increase lock duration
            else:
                assert unlock_week > block.timestamp + MAX_LOCK_DURATION, "only decrease to >= 4y"  # dev: can only decrease to ≥4 years
            new_lock.end = unlock_week

    # create lock
    if old_lock.amount == 0 and old_lock.end == 0:
        assert msg.sender == user, "you can only create a lock for yourself"  # dev: you can only create a lock for yourself
        assert amount >= 10 ** 18, "minimum amount is 1 YFI"  # dev: minimum amount is 1 YFI
        assert unlock_week != 0, "must specify unlock time in the future"  # dev: must specify unlock time in the future
    # modify lock
    else:
        assert old_lock.end > block.timestamp, "lock expired"  # dev: lock expired
    ...
    if amount > 0:
        assert YFI.transferFrom(msg.sender, self, amount), "fail yfi transfer"
...
def withdraw() -> Withdrawn:
    ...
    if penalty > 0:
        assert YFI.approve(self.REWARD_POOL.address, penalty)
        assert self.REWARD_POOL.burn(penalty)
